...
// Unofficial modification for author field see: http://www.dynamicdrive.com/forums/showthread.php?p=268599#post268599
...
gfeedfetcher.prototype.init=function(){
	...
	for (var i=0; i<this.feedurls.length; i++){ //loop through the specified RSS feeds' URLs
		...
		var items_to_show=Math.min(this.feedlimit,20)
...
gfeedfetcher._formatdate=function(datestr, showoptions){
	...
	var parseddate=(showoptions.indexOf("datetime")!=-1)? itemdate.toLocaleDateString('en-US',options) : (showoptions.indexOf("date")!=-1)? itemdate.toLocaleDateString() : (showoptions.indexOf("time")!=-1)? itemdate.toLocaleTimeString() : ""
...
gfeedfetcher.prototype._signaldownloadcomplete=function(){
	...
	if (this.feedsfetched==this.feedurls.length) //if all feeds fetched
		this._displayresult(this.feeds,this.feedlimit) //display results
}


gfeedfetcher.prototype._displayresult=function(feeds,feedlimit){
	var rssoutput=(this.itemcontainer=="<li>")? "<ul>\n" : ""
	gfeedfetcher._sortarray(feeds, this.sortstring)
	for (var i=0; i<Math.min(feedlimit,feeds.length); i++){
		var itemtitle="<br> <a href=\"" + feeds[i].link + "\" target=\"" + this.linktarget + "\" class=\"titlefield\">" + feeds[i].title + "</a>"
		var itemlabel=/label/i.test(this.showoptions)? '<span class="labelfield">'+this.feeds[i].ddlabel+'</span>' : " "
		var itemdate=gfeedfetcher._formatdate(feeds[i].publishedDate, this.showoptions)
		var itemdescription=/description/i.test(this.showoptions)? "<br />"+feeds[i].content : /snippet/i.test(this.showoptions)? "<br />"+feeds[i].contentSnippet  : ""
		var itemauthor = /author/i.test(this.showoptions)? "<br /><span class='authorfield'> "+(feeds[i].author || feeds[i].copyright || feeds.author || feeds.copyright || 'unavailable')+ "</span>": '';
		rssoutput+=this.itemcontainer + itemdate + " " + itemlabel + " " + itemtitle + itemdescription + itemauthor + this.itemcontainer.replace("<", "</") + "\n\n"
	}
